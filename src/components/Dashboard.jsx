...
import { Zap, Download, Globe, Magnet } from 'lucide-react';

export default function Dashboard({ onAddDownload }) {
  const { downloads, stats } = useDownloads();
  const [activeTab, setActiveTab] = useState('all'); // 'all', 'http', 'torrent'
        ...
        {activeTab === 'torrent' && torrentDownloads.length > 0 && (
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
              <SpeedChart title="Peers" data={stats.torrentStats.peers} color="#f59e0b" />
              <SpeedChart title="Seeds" data={stats.torrentStats.seeds} color="#8b5cf6" />
            </div>
          </>
        )}
        ...
        {activeTab === 'all' && downloads.length > 0 && (
          <>
            ...
            {(torrentDownloads.length > 0) && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                <SpeedChart title="Peers" data={stats.peers} color="#f59e0b" />
                <SpeedChart title="Seeds" data={stats.seeds} color="#8b5cf6" />
              </div>
            )}
          </>
        )}
